...
def extract_evidence_gates(model, rel_ids: np.ndarray, device: torch.device) -> Tuple[np.ndarray, np.ndarray]:
    ...
    gates = torch.sigmoid(model.rcem.gate(gate_input)).detach().cpu().numpy()
    effective = gates.copy()
    effective[:, 0] *= float(model.rcem.path_strength)
    effective[:, 1] *= float(model.rcem.type_strength)
...
def main():
    ...
    for idx, r in enumerate(rel_ids_for_fig):
        gate_rows.append({
            ...
            "relation": rel_names.get(int(r), f"rel_{int(r)}"),
            "relation_group": rel_group[int(r)],
            "path_gate": float(gates[idx, 0]),
            "role_gate": float(gates[idx, 1]),
    ...
    write_csv(
        ...
        gate_rows,
        ["rel_id", "relation", "relation_group", "path_gate", "role_gate",
    ...
    # Rename gate_1/gate_2 to path_gate/role_gate for readability.
    renamed_gate_rows = []
    for row in gate_group_rows:
        renamed_gate_rows.append({
            "relation_group": row["relation_group"],
            "count": row["count"],
            "path_gate": row.get("gate_1", np.nan),
            "role_gate": row.get("gate_2", np.nan),
        })
    write_csv(
        os.path.join(args.out_dir, "evidence_gates_by_type.csv"),
        renamed_gate_rows,
        ["relation_group", "count", "path_gate", "role_gate"]
    )
    if renamed_gate_rows:
        labels = [r["relation_group"] for r in renamed_gate_rows]
        values = np.asarray([[r["path_gate"], r["role_gate"]] for r in renamed_gate_rows], dtype=np.float32)
        plot_grouped_bars(
            labels,
            values,
            ["Path gate", "Role gate"],
