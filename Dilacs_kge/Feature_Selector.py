"""
@FileName：Feature_Selector2.py
@Description：
@Author：zhangyt\n
@Time：2025/2/24 14:36
"""
import torch
...
class SpatialAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        output = self.conv(
            result)  # 然后重新降维为1维:(B,1,H,W); 在这里并没有按照模型里的的方式先MLP,然后再Add; 而是先concat,再Conv; 实际含义是一致的,就是实现方式不一致。
        return output
